...
class Nodes {
	...
	first() {
		...
		if (this._items.length > 1) throw new WebError({ message: "Mutliple children." });
		return this._items[0];
	};

	push(node) {
		
		if (!node) throw new WebError({ message: "Node is not defined." });
		if (!node.token == undefined) throw new WebError({ message: "Token is not defined." });
		...
		if (!existingNode) {
			...
			if (node.router) { //node could be a leaf
				if (existingNode.router) throw new WebError({ message: "Multiple end route.", data: { first: existingNode.router.route, second: node.router.route }});
	...
	findByToken(node) {
		...
		throw new WebError({ message: "Multiple token.", data: { tokens: results }});
	}
	...
	isItForMe(node) {
		...
		for (let i = 0; i < count; i++) {
			...
			if (!first) {
				...
				let item = result.node.nodes.isItForMe(first);
				if (item) {
